  ...
  return (
    <nav
      className={cn(
        ...
        scrolled
          ? "bg-surface-50/90 dark:bg-surface-200/90 backdrop-blur-xl border border-border-default shadow-soft-lg"
          : "bg-surface-50/70 dark:bg-surface-200/60 backdrop-blur-md border border-border-default/60",
      )}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-lg text-text-muted hover:text-brand-amber-600 hover:bg-surface-200 dark:hover:bg-surface-300 transition-colors"
            ...
            <button
              ...
              className="p-2 rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-200 dark:hover:bg-surface-300 transition-colors flex items-center gap-1"
                  ...
                  {dropdownOpen && (
                    <>
                      ...
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -8 }}
                        className={cn(
                          "absolute mt-2 w-48 py-1 bg-surface-50 dark:bg-surface-200 rounded-xl shadow-soft-lg border border-border-default z-20",
                      ...
                      >
                        <Link
                          to="/admin"
                          onClick={() => setDropdownOpen(false)}
                          className="flex items-center gap-2 px-4 py-2 text-sm text-text-secondary hover:bg-surface-100 dark:hover:bg-surface-300 hover:text-text-primary"
                        >
                        ...
                        <button
                          ...
                          className="flex items-center gap-2 w-full px-4 py-2 text-sm text-brand-red-600 hover:bg-brand-red-50 dark:hover:bg-brand-red-900/20"
                        >
            ...
            <button
              ...
              className="md:hidden p-2 rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-200 dark:hover:bg-surface-300"
            >
              {mobileOpen ? (
                <X className="w-5 h-5" />
              ) : (
              ...
              {isAuthenticated && isAdmin && (
                <>
                  ...
                  <Link
                    to="/admin"
                    onClick={() => setMobileOpen(false)}
                    className="block px-4 py-2.5 text-sm text-text-secondary hover:text-text-primary hover:bg-surface-100 dark:hover:bg-surface-300 rounded-lg"
                  >
                    {t.nav.adminPanel}
                  </Link>
                  <button
                    onClick={handleLogout}
                    className="block w-full text-start px-4 py-2.5 text-sm text-brand-red-600 hover:bg-brand-red-50 dark:hover:bg-brand-red-900/20 rounded-lg"
                  >
                    {t.nav.logout}
                  </button>
                </>
