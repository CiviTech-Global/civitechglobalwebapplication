...
export default function OpportunityDetailPage() {
  ...
  const applyMutation = useMutation({
    mutationFn: async () => {
      await api.post(`/opportunities/${opportunity!.id}/apply`, {
        coverLetter,
        resumeUrl,
      });
    },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link
        to="/careers"
        className="inline-flex items-center gap-2 text-sm text-text-muted hover:text-brand-green-500 transition-colors mb-8"
      >
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          <Badge
            variant={opportunity.opportunityType === "JOB" ? "info" : "success"}
            className="gap-1"
          >
        ...
        <div className="flex items-center gap-6 text-sm text-text-muted mb-6">
          ...
          <span className="flex items-center gap-1">
            <Clock className="w-4 h-4" /> {opportunity.duration}
          </span>
        </div>
