...
export default function AdminOrderDetail() {
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => {
      ...
      toast(t.orders.updateStatus, "success");
    },
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Badge variant={statusVariant[order.status]}>
          {
            t.orders.statusLabels[
              order.status as keyof typeof t.orders.statusLabels
            ]
          }
        </Badge>
      </div>
      ...
      <Card className="mb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <p className="text-text-muted">{t.admin.userManagement.name}</p>
          ...
          <div>
            <p className="text-text-muted">{t.orders.total}</p>
            <p className="font-bold text-brand-green-600">
              {formatPrice(order.total)}
            </p>
          </div>
          <div>
            <p className="text-text-muted">{t.orders.items}</p>
            <p className="text-text-primary">{order.items?.length || 0}</p>
          </div>
        </div>
      </Card>
            ...
            >
              <div>
                ...
                <p className="text-sm text-text-muted">
                  {t.orders.quantity}: {item.quantity} x{" "}
        ...
        <Card>
          ...
          <div className="flex gap-2">
            {currentIdx < statusFlow.length - 1 && (
              <Button
                onClick={() => updateStatus.mutate(statusFlow[currentIdx + 1])}
              ...
              >
                {
                  t.orders.statusLabels[
                    statusFlow[
                      currentIdx + 1
                    ] as keyof typeof t.orders.statusLabels
                  ]
                }
              </Button>
            )}
