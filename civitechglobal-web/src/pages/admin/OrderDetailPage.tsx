    ...
    <div>
      ...
      <Card className="mb-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div><p className="text-dark-500">{t.admin.userManagement.name}</p><p className="font-medium text-dark-900 dark:text-white">{order.user?.firstName} {order.user?.lastName}</p></div>
          <div><p className="text-dark-500">{t.orders.date}</p><p className="text-dark-900 dark:text-white">{formatDate(order.createdAt)}</p></div>
          <div><p className="text-dark-500">{t.orders.total}</p><p className="font-bold text-primary-600">{formatPrice(order.total)}</p></div>
          <div><p className="text-dark-500">{t.orders.items}</p><p className="text-dark-900 dark:text-white">{order.items?.length || 0}</p></div>
        </div>
      </Card>

      <Card className="mb-6">
        <h2 className="font-semibold text-dark-900 dark:text-white mb-4">{t.orders.items}</h2>
      ...
      {order.status !== 'COMPLETED' && order.status !== 'CANCELLED' && (
        <Card>
          <h2 className="font-semibold text-dark-900 dark:text-white mb-4">{t.orders.updateStatus}</h2>
