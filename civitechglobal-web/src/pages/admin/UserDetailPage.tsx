...
export default function AdminUserDetail() {
  ...
  type UserDetail = User & { _count?: Record<string, number> };
  ...
  const updateRole = useMutation({
    ...
    onSuccess: () => {
      ...
      toast(t.admin.userManagement.changeRole, "success");
    },
  ...
  const deleteUser = useMutation({
    ...
    onSuccess: () => {
      toast(t.admin.userManagement.deleteUser, "success");
    },
    onError: () => toast(t.error, "error"),
  });
  ...
  if (!user)
    return <div className="text-center py-8 text-text-muted">{t.noData}</div>;

  const BackArrow = isRtl ? ArrowRight : ArrowLeft;
  const effectivePerms =
    localPerms !== null ? localPerms : user.permissions || [];
  const permsDirty =
    localPerms !== null &&
    JSON.stringify(localPerms) !== JSON.stringify(user.permissions || []);
    ...
    <div>
      ...
      <Card className="mb-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <Badge variant={roleVariant[user.role]}>
              {
                t.admin.userManagement.roles[
                  user.role as keyof typeof t.admin.userManagement.roles
                ]
              }
            </Badge>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <p className="text-text-muted">{t.admin.userManagement.joinDate}</p>
            <p className="text-text-primary">{formatDate(user.createdAt)}</p>
          </div>
          <div>
            <p className="text-text-muted">{t.admin.orders}</p>
            <p className="text-text-primary">{user._count?.orders || 0}</p>
          </div>
          <div>
            <p className="text-text-muted">{t.admin.tickets}</p>
            <p className="text-text-primary">{user._count?.tickets || 0}</p>
          </div>
          <div>
            <p className="text-text-muted">{t.admin.applications}</p>
            <p className="text-text-primary">
              {user._count?.opportunityApplications || 0}
            </p>
          </div>
        </div>
      </Card>
      ...
      {isSuperAdmin && user.id !== currentUser?.id && (
        <Card>
          ...
          <div className="flex items-center gap-2 mb-4">
            <span className="text-sm text-text-muted me-2">
              {t.admin.userManagement.changeRole}:
            </span>
            {["USER", "ADMIN", "SUPER_ADMIN"].map((role) => (
              ...
              >
                {
                  t.admin.userManagement.roles[
                    role as keyof typeof t.admin.userManagement.roles
                  ]
                }
              </Button>
            ))}
          ...
          <Button
            ...
            className="gap-2"
            onClick={() => {
              if (confirm(t.confirmDelete)) deleteUser.mutate();
            }}
          >
