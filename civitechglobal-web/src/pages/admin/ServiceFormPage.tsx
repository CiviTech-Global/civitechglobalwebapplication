...
export default function ServiceFormPage() {
  ...
  return (
    <div>
      <Link to="/admin/services" className="inline-flex items-center gap-2 text-sm text-text-muted hover:text-brand-green-600 mb-6">
        <BackArrow className="w-4 h-4" /> {t.services.backToServices}
      </Link>
      <h1 className="text-2xl font-bold text-text-primary mb-6">{isEdit ? t.admin.serviceForm.editTitle : t.admin.serviceForm.createTitle}</h1>
      ...
      <Card className="max-w-2xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            <span className="text-text-secondary">{t.admin.serviceForm.isActive}</span>
          </label>
