  ...
  const columns = [
    {
      ...
      render: (tk: Ticket) => <span className="font-medium">{tk.subject}</span>,
    },
    {
      key: "user",
      header: t.admin.userManagement.name,
      render: (tk: Ticket) =>
        tk.user ? `${tk.user.firstName} ${tk.user.lastName}` : tk.email,
    },
    {
      key: "status",
        ...
        <Badge variant={statusVariant[tk.status]}>
          {
            t.tickets.statusLabels[
              tk.status as keyof typeof t.tickets.statusLabels
            ]
          }
        </Badge>
      ),
        ...
        <Badge variant={priorityVariant[tk.priority]}>
          {
            t.tickets.priorityLabels[
              tk.priority as keyof typeof t.tickets.priorityLabels
            ]
          }
        </Badge>
      ),
    ...
    {
      key: "messages",
      header: t.tickets.messages,
      render: (tk: Ticket) => tk._count?.messages || 0,
    },
    {
      key: "createdAt",
      header: t.tickets.date,
      render: (tk: Ticket) => formatDate(tk.createdAt),
    },
    {
      key: "actions",
      header: "",
