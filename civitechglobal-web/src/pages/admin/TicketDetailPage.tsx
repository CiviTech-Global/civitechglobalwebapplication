...
export default function AdminTicketDetail() {
  ...
  if (!ticket) return <div className="text-center py-8 text-text-muted">{t.noData}</div>;

  const BackArrow = isRtl ? ArrowRight : ArrowLeft;

  return (
    <div>
      <Link to="/admin/tickets" className="inline-flex items-center gap-2 text-sm text-text-muted hover:text-brand-green-600 mb-6">
      ...
      <Card className="mb-6">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-sm text-text-muted me-2">{t.tickets.updateStatus}:</span>
          ...
          <span className="text-sm text-text-muted ms-4 me-2">{t.tickets.priority}:</span>
      ...
      {/* Messages */}
      <Card className="mb-6">
        <h2 className="font-semibold text-text-primary mb-4">{t.tickets.messages}</h2>
        <div className="space-y-4 max-h-96 overflow-y-auto">
          {ticket.messages?.map((msg) => (
            <div key={msg.id} className={`p-3 rounded-lg ${msg.isStaff ? 'bg-brand-green-900/20 ms-8' : 'bg-surface-200 me-8'}`}>
