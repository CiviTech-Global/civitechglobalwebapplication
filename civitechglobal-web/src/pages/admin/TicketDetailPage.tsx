...
export default function AdminTicketDetail() {
  ...
  const updateStatus = useMutation({
    mutationFn: async (data: Record<string, string>) => {
      await api.put(`/tickets/${id}/status`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-ticket", id] });
      toast(t.tickets.updateStatus, "success");
    },
    onError: () => toast(t.admin.contentManagement.saveFailed, "error"),
  });
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Badge>
            {
              t.tickets.priorityLabels[
                ticket.priority as keyof typeof t.tickets.priorityLabels
              ]
            }
          </Badge>
        </div>
      ...
      <Card className="mb-6">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-sm text-text-muted ms-4 me-2">
            {t.tickets.priority}:
          </span>
          {["LOW", "MEDIUM", "HIGH", "URGENT"].map((p) => (
            <Button
              ...
              onClick={() => updateStatus.mutate({ priority: p })}
            >
              {
                t.tickets.priorityLabels[
                  p as keyof typeof t.tickets.priorityLabels
                ]
              }
            </Button>
          ))}
