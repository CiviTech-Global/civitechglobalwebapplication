...
export default function AdminDashboard() {
  ...
  if (!isSuperAdmin) {
    ...
    return (
      <div>
        ...
        <Card className="mb-6">
          <h2 className="font-semibold text-text-primary mb-2">{t.admin.welcomeBack}</h2>
          <p className="text-text-muted text-sm">{t.admin.permissions?.description || 'Manage your assigned modules from the sidebar.'}</p>
        </Card>
  ...
  const stats = [
    ...
    { icon: Package, label: t.admin.stats.totalProducts, value: data.stats.totalProducts, color: 'text-brand-amber-500 bg-brand-amber-50' },
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          <h2 className="font-semibold text-text-primary mb-4">{t.admin.recentOrders}</h2>
          <div className="space-y-3">
            {data.recentOrders.map((o: any) => (
              <div key={o.id} className="flex items-center justify-between py-2 border-b border-border-default last:border-0">
                <div>
                  <p className="text-sm font-medium text-text-primary">{o.user?.firstName} {o.user?.lastName}</p>
                  <p className="text-xs text-text-muted">{formatDate(o.createdAt)}</p>
                </div>
                <div className="text-end">
                  <p className="text-sm font-medium text-text-primary">{formatPrice(o.total)}</p>
        ...
        <Card>
          <h2 className="font-semibold text-text-primary mb-4">{t.admin.recentTickets}</h2>
          <div className="space-y-3">
            {data.recentTickets.map((tk: any) => (
              <div key={tk.id} className="flex items-center justify-between py-2 border-b border-border-default last:border-0">
                <div>
                  <p className="text-sm font-medium text-text-primary">{tk.subject}</p>
                  <p className="text-xs text-text-muted">{formatDate(tk.createdAt)}</p>
                </div>
