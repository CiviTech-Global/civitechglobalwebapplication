...
export default function RoleFormPage() {
  ...
  const currentPermissions = permissions ?? existing?.permissions ?? [];

  const mutation = useMutation({
    mutationFn: async () => {
      const payload = { name: currentName, permissions: currentPermissions };
      if (isEdit) {
  ...
  const togglePermission = (perm: string) => {
    setPermissions((prev) => {
      const base = prev ?? existing?.permissions ?? [];
  ...
  const toggleAll = () => {
    setPermissions((prev) => {
      const base = prev ?? existing?.permissions ?? [];
      return base.length === ALL_PERMISSIONS.length ? [] : [...ALL_PERMISSIONS];
    });
  };
