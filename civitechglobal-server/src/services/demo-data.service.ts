...
const DEMO_USERS = [
  {
    email: 'alice@demo.civitechglobal.com',
    username: 'demo_alice',
    firstName: 'Alice',
    lastName: 'Johnson',
    role: 'USER' as const,
  ...
  {
    email: 'bob@demo.civitechglobal.com',
    username: 'demo_bob',
    firstName: 'Bob',
    lastName: 'Williams',
    role: 'USER' as const,
  ...
  {
    email: 'carol@demo.civitechglobal.com',
    username: 'demo_carol',
    firstName: 'Carol',
    lastName: 'Martinez',
    role: 'USER' as const,
  ...
  {
    email: 'dave@demo.civitechglobal.com',
    username: 'demo_dave',
    firstName: 'Dave',
    lastName: 'Brown',
    role: 'ADMIN' as const,
  ...
  {
    email: 'eve@demo.civitechglobal.com',
    username: 'demo_eve',
    firstName: 'Eve',
    lastName: 'Davis',
    role: 'USER' as const,
...
export async function seedDemoData() {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (existingProductCount === 0) {
      products = await Promise.all(DEMO_PRODUCTS.map((p) => tx.product.create({ data: p })));
    }
    ...
    if (existingServiceCount === 0) {
      services = await Promise.all(DEMO_SERVICES.map((s) => tx.service.create({ data: s })));
    }

    // 4. Seed demo opportunities
    const opportunities = await Promise.all(DEMO_OPPORTUNITIES.map((o) => tx.opportunity.create({ data: o })));

    // 5. Seed demo orders (need products to exist)
    const allProducts =
      products.length > 0 ? products : await tx.product.findMany({ select: { id: true, price: true }, take: 7 });
