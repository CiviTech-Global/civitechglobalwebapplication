  ...
  await runAsSystem(async () => {
    ...
    if (!superAdmin) {
      superAdmin = await prisma.user.create({
        data: {
          email: encrypt(SUPER_ADMIN_EMAIL),
          emailHash: superAdminEmailHash,
          username: 'superadmin',
          password: superAdminHashed,
          firstName: encrypt(SUPER_ADMIN_FIRST_NAME),
          lastName: encrypt(SUPER_ADMIN_LAST_NAME),
          role: 'SUPER_ADMIN',
    ...
    if (!demoAdmin) {
      demoAdmin = await prisma.user.create({
        data: {
          email: encrypt(DEMO_ADMIN_EMAIL),
          emailHash: demoAdminEmailHash,
          username: 'admin',
          password: adminHashed,
          firstName: encrypt(DEMO_ADMIN_FIRST_NAME),
          lastName: encrypt(DEMO_ADMIN_LAST_NAME),
          role: 'ADMIN',
    ...
    if (!demoUser) {
      demoUser = await prisma.user.create({
        data: {
          ...
          emailHash: demoUserEmailHash,
          password: userPassword,
          firstName: encrypt(USER_FIRST_NAME),
          lastName: encrypt(USER_LAST_NAME),
          role: 'USER',
        },
      });
    }
    ...
    const demoTickets = await Promise.all([
      prisma.ticket.create({
        data: {
          ...
          email: encrypt(USER_EMAIL),
          emailHash: hashForSearch(normalizeEmail(USER_EMAIL)),
          category: 'SUPPORT',
          status: 'OPEN',
          priority: 'MEDIUM',
        },
      ...
      prisma.ticket.create({
        data: {
          ...
          email: encrypt('partner@example.com'),
          emailHash: hashForSearch(normalizeEmail('partner@example.com')),
          category: 'SALES',
          status: 'OPEN',
          priority: 'LOW',
        },
    ...
    console.log('----------------------------------------');
    console.log('Super Admin, Demo Admin, and Demo User created.');
    console.log('----------------------------------------');
